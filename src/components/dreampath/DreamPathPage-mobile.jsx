...
/**
 * DreamPathPage-mobile 컴포넌트
 *
 * 이 컴포넌트는 모바일용 DreamPath 페이지를 구현합니다.
 * 주요 기능:
...
function DreamPathPageMobile({ language = 'ko' }) {
  ...
  const handlePrintClick = () => {
    ...
    if (language === 'ko') {
      ...
      const folderName = LANGUAGE_FOLDER_MAP[language] || language;
      pdfUrl = `/DreamPath/Multilingual/${folderName}/DreamPath-Pdf/dreampath-${language}.pdf`;
    }
  ...
  const handleDownloadClick = () => {
    const pdfUrl = getDreamPathPdfPath(language);
    const suggestedName = language === 'ko' ? 'dreampath-ko.pdf' : `dreampath-${language}.pdf`;
